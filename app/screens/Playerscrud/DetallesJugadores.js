...
export default function DetallesJugadores(props) {
    ...
    const deleteJugador = async () => {
        const dbRef = Firebase.db.collection("jugadores").doc(props.route.params.jugadorId);
        await dbRef.delete();
    ...
    const alertaConfirmacionBorrar = () => {
        Alert.alert("borrar el jugador", "¿Estás seguro que quieres borrar el jugador?", [
            {text: "SI", onPress: () => deleteJugador()},
            {text: "NO", onPress: () => toastRef.current.show("No borró el jugador", 3000)},
        ])
    }
